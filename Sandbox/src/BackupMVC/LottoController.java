...
public class LottoController {
	...
	public LottoController(LottoModel model, LottoView view) {
		...
		serviceLocator.getLogger().info("LottoController gestartet");
		...
		view.btnEnglish.setOnAction(new changeLanguage());
		view.btnDeutsch.setOnAction(new changeLanguage());
		...
		serviceLocator.getLogger().info("LottoController initialized");
			...
			view.lblStatus.setText("");
			 ...
			 if(sourceSelected){ 
				 if(anzGew < view.MAXCHOICE){
					 ...
					 view.lblStatus.setText(t.getString("status.maxChoice"));
		...
		public void handle(ActionEvent event) {
			...
			if(anzTips<model.MAXTIP){
				...
				if(anzGew >= view.MAXCHOICE){
					...
					view.lblStatus.setText(t.getString("status.notAll"));
				}
			    model.bet += model.tipPrice;
			}else{
				view.lblStatus.setText(t.getString("status.noMoreTips"));
			}
		    updateChoiceProgress();
		}
	}
	...
	public class startZiehung implements EventHandler<ActionEvent>{
		...
		public void handle(ActionEvent event) {
			view.screenNR++;
			view.lblStatus.setText("");
			view.lblTitle.setText(t.getString("title.drawing"));
	...
	public class removeTip implements EventHandler<ActionEvent>{
		...
		public void handle(ActionEvent event) {
			...
			updateChoiceProgress();
				...
				view.lblStatus.setText(t.getString("status.selectTip"));
			}
			model.bet -= model.tipPrice;
	...
	public class StartChoice implements EventHandler<ActionEvent>{
		...
		public void handle(ActionEvent arg0) {
			view.screenNR++;
			view.lblStatus.setText("");
			view.lblTitle.setText(t.getString("title.choice"));
	...
	public class changeLanguage implements EventHandler<ActionEvent>{
		...
		public void handle(ActionEvent event) {
			...
			if(temp.getId() == "btnDE"){
				serviceLocator.getConfiguration().setLocalOption("Language", "de");
				serviceLocator.getLogger().info("LanguageConfig changed to DE");
				
			}else{
				serviceLocator.getConfiguration().setLocalOption("Language", "en");
				serviceLocator.getLogger().info("LanguageConfig changed to EN");
			}
			serviceLocator.setTranslator(new Translator(serviceLocator.getConfiguration().getOption("Language")));
			t = serviceLocator.getTranslator();
